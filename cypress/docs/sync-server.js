...
// ── /run?spec=<path>&mode=headed|headless ─────────────────────────────────────
function handleRun(req, res) {
  ...
  // Build cypress args based on mode
  ...
  const processLine = (rawLine) => {
    ...
    // "Running: cypress/e2e/checkout/stripe/shopify-stripe-card.cy.js" → file started
    ...
    if (tblMatch) {
      ...
      specPasses   += pass;
      specFailures += fail;
      send({ status: 'spec-done', file: fileForEvent, total, pass, fail, specTotal, specPasses, specFailures });
      return;
    }
  ...
  child.on('close', (code) => {
    ...
    // Read the mochawesome HTML report for detailed per-test results
...
const server = http.createServer((req, res) => {
  ...
  const filePath = path.join(
    __dirname,
    pathname === '/' ? 'TEST_CASES.html' : pathname.slice(1)
  );
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  console.log(`    Dashboard →  http://127.0.0.1:${PORT}/TEST_CASES.html`);
  console.log(`    /ping  /sync  /run?spec=…&mode=headed|headless  /reports  /reports/get  /reports/delete\n`);
});

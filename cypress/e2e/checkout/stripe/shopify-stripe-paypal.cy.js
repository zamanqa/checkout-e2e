...
describe('Shopify + Stripe + PayPal Checkout', () => {
  ...
  it('Complete an Order for Shopify Stripe PayPal and Validate in CMS Database', function () {
    ...
    // Step 12: Handle Stripe PayPal test payment page (cross-origin)
    cy.log('--- Step 12: Authorize test payment on Stripe ---');
    cy.url({ timeout: 60000 }).should('include', 'stripe.com');
    cy.url().then((stripeUrl) => {
      cy.log(`Stripe URL: ${stripeUrl}`);
      cy.visit(stripeUrl);
      cy.wait(5000);

      // Authorize test payment on Stripe page
      StripeComponent.authorizeTestPayment();
    });

    // Step 13: Wait for redirect back and verify order confirmation

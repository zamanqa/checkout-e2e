// shopify-stripe-invoice.cy.js - Shopify + Stripe + Pay with Invoice Checkout Test
...
describe('Shopify + Stripe + Pay with Invoice Checkout', () => {
  ...
  it('Complete an Order for Shopify with Invoice Payment and Validate in CMS Database', function () {
    cy.log('========== Shopify Pay with Invoice Checkout Test ==========');
    ...
    // Step 9: Click "Pay with invoice" button (skip actual payment method)
    cy.log('--- Step 9: Click Pay with Invoice ---');
    cy.get("[data-test-id='select-invoice'], [data-test-id='select-offline-invoice']")
      .should('be.visible')
      .click();
    cy.log('✓ Clicked Pay with Invoice');
    cy.wait(3000);
